...
function telefono(tel) {
    ...
    // const filtered_str = cleanup(this.input);
    const filtered_str = cleanup(tel);
    const result = filtered_str.match(regex);

    if (result !== null) {
        const data = setData(result);

        // Valida el numero y retorna el tipo
        const phone_type = getType(data);
...
function setData(result) {
    ...
    let number, mobile_prefix, area_code;
    // Obtengo numero
    const number_list = [9, 15, 12, 21, 17, 19, 22, 24];
    for (let i = 0; i <= number_list.length - 1; i++) {
        if (result[parseInt(number_list[i])] !== undefined) {
            number = result[parseInt(number_list[i])];
            break;
        } else {
            number = false;
        }
    ...
    const mobile_prefix_list = [8, 11, 14, 18, 20];
    for (let i = 0; i <= mobile_prefix_list.length - 1; i++) {
        if (result[parseInt(mobile_prefix_list[i])] !== undefined) {
            mobile_prefix = result[parseInt(mobile_prefix_list[i])];
            break;
        } else {
            mobile_prefix = false;
        }
    ...
    const area_code_list = [13, 10, 7];
    for (let i = 0; i <= area_code_list.length - 1; i++) {
        if (result[parseInt(area_code_list[i])] !== undefined) {
            area_code = result[parseInt(area_code_list[i])];
            break;
        } else {
            area_code = false;
        }
...
function numberFormat(data) {
    ...
    const number = data.number.slice(0, data.number.length - 4) +
        '-' + data.number.slice(-4);
    ...
    const formated_number = d['international'] + space + country + space +
